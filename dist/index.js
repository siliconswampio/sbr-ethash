...
class Ethash {
    ...
    mkcache(cacheSize, seed) {
        ...
        const o = [sbr_util_1.keccak(seed, 512)];
        let i;
        for (i = 1; i < n; i++) {
    ...
    run(val, nonce, fullSize) {
        ...
        const s = sbr_util_1.keccak(Buffer.concat([val, util_1.bufReverse(nonce)]), 512);
    ...
    async _verifyPOW(header) {
        ...
        const result = new sbr_util_1.BN(a.hash);
        return a.mix.equals(mixHash) && sbr_util_1.TWO_POW256.div(difficulty).cmp(result) === 1;
    }
